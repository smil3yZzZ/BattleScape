...
int main(){
    ...
    unsigned char *grayscale = stbi_load("heights2.png", &width, &height, &nrChannels, 3);

    cout << "WIDTH: " << width << endl;
    cout << "NR. CHANNELS: " << nrChannels << endl;
    */
    ...
    for(int i = 0; i < iMax; i++){
        for(int j = 0; j < jMax; j++){
            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j] = j*QUAD_WIDTH;
            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j + 1] = i*QUAD_HEIGHT;
            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j + 2] = 0.0f;
            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j + 3] = gradients[i*(jMax + 1) + j][0];
            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j + 4] = gradients[i*(jMax + 1) + j][1];

            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j + 5] = j*QUAD_WIDTH;
            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j + 6] = i*QUAD_HEIGHT + QUAD_HEIGHT;
            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j + 7] = 0.0f;
            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j + 8] = gradients[(i + 1)*(jMax + 1) + j][0];
            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j + 9] = gradients[(i + 1)*(jMax + 1) + j][1];

            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j + 10] = j*QUAD_WIDTH + QUAD_WIDTH;
            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j + 11] = i*QUAD_HEIGHT;
            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j + 12] = 0.0f;
            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j + 13] = gradients[i*(jMax + 1) + j + 1][0];
            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j + 14] = gradients[i*(jMax + 1) + j + 1][1];

            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j + 15] = j*QUAD_WIDTH + QUAD_WIDTH;
            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j + 16] = i*QUAD_HEIGHT + QUAD_HEIGHT;
            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j + 17] = 0.0f;
            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j + 18] = gradients[(i + 1)*(jMax + 1) + j + 1][0];
            vertices[i*jMax*BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD + BUFFER_VERTEX_SIZE*VERTICES_PER_QUAD*j + 19] = gradients[(i + 1)*(jMax + 1) + j + 1][0];
    ...
    */
    ...
    glm::mat4 proj = glm::ortho(0.0f, 800.0f, 0.0f, 600.0f, -100.0f, 100.0f);
    ...
    // gradient attribute
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, BUFFER_VERTEX_SIZE * sizeof(float), (void*)(3 * sizeof(float)));
    glEnableVertexAttribArray(1);

    Shader textureShader("shaders/textureVertexShader.vert", "shaders/textureFragmentShader.frag");
